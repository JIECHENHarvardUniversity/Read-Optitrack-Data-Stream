...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	printf("[ReadCamera] Sending Test Request\n");
	void* response;
	int nBytes;
	iResult = theClient->SendMessageAndWait("TestRequest", &response, &nBytes);
	if (iResult == ErrorCode_OK)
	{
		printf("[ReadCamera] Received: %s", (char*)response);
	}

	// Retrieve Data Descriptions from server
	printf("\n\n[ReadCamera] Requesting Data Descriptions...");
	sDataDescriptions* pDataDefs = NULL;
	int nBodies = theClient->GetDataDescriptions(&pDataDefs);
	if(!pDataDefs)
	{
		printf("[ReadCamera] Unable to retrieve Data Descriptions.");
	}
	else
	{
        printf("[ReadCamera] Received %d Data Descriptions:\n", pDataDefs->nDataDescriptions );
	...
	int filename_yyyymmdd=(ltm->tm_year+1900)*10000+(ltm->tm_mon+1)*100+ltm->tm_mday;
	int filename_hhmmss=ltm->tm_hour*10000+ltm->tm_min*100+ltm->tm_sec;
	
    sprintf(filename,"Data_%d_%d.txt",filename_yyyymmdd,filename_hhmmss);
	...
	if(argc > 3)
		...
		sprintf(szFile, "%s\\%s",szFolder,filename);
	fp = fopen(szFile, "w");
	if(!fp)
	{
	...
	while(c =_getch())
	{
		switch(c)
		{
            ...
            case 'd' :                          // disconnect
                ...
                if (iResult == ErrorCode_OK)
                    printf("[ReadCamera] Disconnected");
                break;
			default:
				break;
		}
...
int CreateClient(int iConnectionType)
{
    ...
    else
    {
        ...
        printf("[ReadCamera] Server application info:\n");
...
{
	// Other Markers
	fprintf(fp,"Other Markers [Count=%d]\t", data->nOtherMarkers);
	for(int i=0; i < data->nOtherMarkers; i++)
	{
		fprintf(fp,"Other Marker %d : %3.5f\t%3.5f\t%3.5f\t",
			i,
			data->OtherMarkers[i][2],
			data->OtherMarkers[i][0],
			data->OtherMarkers[i][1]);
